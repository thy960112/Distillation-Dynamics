...
class DistillationLoss(nn.Module):
    ...
    def __init__(self, base_criterion: torch.nn.Module, teacher_model: torch.nn.Module, args):
        ...
        # Initialize projectors and batch normalization layers
        self.projectors = nn.ModuleList()
        self.bn_student = nn.ModuleList()
        self.bn_teacher = nn.ModuleList()
        
        # We'll initialize the projectors when we first see the feature dimensions
        self.projectors_initialized = False

    def _initialize_projectors(self, block_outs_s, block_outs_t):
        """Initialize projectors based on the actual feature dimensions"""
        ...
        for F_s, F_t in zip(block_outs_s, block_outs_t):
            ...
            projector = nn.Linear(C_s, C_t)
            
            
            # Batch normalization for both student and teacher
            bn_s = nn.BatchNorm2d(C_t)
            bn_t = nn.BatchNorm2d(C_t)
            
            self.projectors.append(projector)
            self.bn_student.append(bn_s)
            self.bn_teacher.append(bn_t)
        
        # Move to the same device as the model
        device = next(self.parameters()).device
        self.projectors = self.projectors.to(device)
        self.bn_student = self.bn_student.to(device)
        self.bn_teacher = self.bn_teacher.to(device)
    ...
    def forward(self, inputs, outputs, labels):
        ...
        # Initialize projectors on first forward pass
        if not self.projectors_initialized:
            self._initialize_projectors(block_outs_s, block_outs_t)

        loss_base = (1-self.alpha) * base_loss
        loss_fft = fft_loss_with_projector(block_outs_s, block_outs_t, 
                                         self.projectors, self.bn_student, self.bn_teacher)
...
def fft_loss_with_projector(block_outs_s, block_outs_t, projectors, bn_student, bn_teacher):
    losses = []
    for i, (F_s, F_t) in enumerate(zip(block_outs_s, block_outs_t)):
        loss_fft = layer_fft_loss_with_projector(F_s, F_t, projectors[i], bn_student[i], bn_teacher[i])
...
def layer_fft_loss_with_projector(F_s, F_t, projector, bn_s, bn_t):
    """
    Compute FFT loss with learnable projector and batch normalization
    """
    B, C_s, H, W = F_s.shape
    _, C_t, _, _ = F_t.shape

    
    # Reshape for linear layer: (B, C_s, H, W) -> (B, H, W, C_s) -> (B*H*W, C_s)
    F_s_reshaped = F_s.permute(0, 2, 3, 1).reshape(-1, C_s)
    F_s_projected = projector(F_s_reshaped)
    # Reshape back: (B*H*W, C_t) -> (B, H, W, C_t) -> (B, C_t, H, W)
    F_s = F_s_projected.reshape(B, H, W, C_t).permute(0, 3, 1, 2)
