...
def main(args):
    ...
    criterion = DistillationLoss(base_criterion, teacher_model, args)
    criterion.to(device)

    # Create optimizer with model parameters
    optimizer = create_optimizer(args, model_without_ddp)
    
    # Add criterion parameters to optimizer if using distillation
    if args.distillation_type != 'none':
        # Add criterion parameters to the optimizer's param_groups
        criterion_params = list(criterion.parameters())
        if criterion_params:  # Only add if there are parameters to add
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            # Save checkpoint every 30 epochs with unique filename
            if (epoch + 1) % 30 == 0:
                epoch_checkpoint_path = output_dir / f'checkpoint_epoch_{epoch + 1}.pth'
                utils.save_on_master(checkpoint_data, epoch_checkpoint_path)
                print(f'Checkpoint saved at epoch {epoch + 1}: {epoch_checkpoint_path}')
